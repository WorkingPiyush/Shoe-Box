...
function ImgCard({ props }) {
    ...
    return (
        <div onClick={() => navigate(`/${props.gender}/${props.id}`)} className='flex p-1'>
            <div className='bg-white p-1 m-1 w-38 h-78 border border-dashed rounded-4xl cursor-pointer hover:shadow-lg hover:scale-101 md:h-68 md:w-70 md:p-4'>
                <img className='p-1 h-[40%] w-50 md:h-[50%] md:w-full md:p-3' style={{
                    objectFit: 'contain',
                }} src={props.image[0]} alt="shoeImg" />
                <p className='font-bold text-gray-400 text-center uppercase'>{props.brand}</p>
                <h1 className='text-center font-bold text-xl text-wrap md:text-sm'>{props.name}</h1>
